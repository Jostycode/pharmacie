...
function ProduitsEtStock() {
  ...
  const loadToutesDonnees = useCallback(async () => {
    ...
    try {
      const [resProd, resAlertes] = await Promise.all([
        axios.get("http://192.168.100.34:3000/api/produit", getAxiosConfig()),
        axios.get("http://192.168.100.34:3000/api/lots/alertes-peremption", getAxiosConfig())
      ]);
  ...
  const loadLotsDuProduit = async (idProduit) => {
    try {
      ...
      const res = await axios.get(`http://192.168.100.34:3000/api/lots/produit/${idProduit}`, getAxiosConfig());
  ...
  const submitProduit = async (e) => {
    ...
    try {
      if (editId) {
        await axios.put(`http://192.168.100.34:3000/api/produit/update/${editId}`, payload, getAxiosConfig());
      } else {
        await axios.post("http://192.168.100.34:3000/api/produit", payload, getAxiosConfig());
      }
      resetFormProduit();
      loadToutesDonnees();
  ...
  const handleDeleteProduit = async (id) => {
    ...
    if (code === "CONFIRMER") {
      try {
        await axios.delete(`http://192.168.100.34:3000/api/produit/delete/${id}`, getAxiosConfig());
        loadToutesDonnees();
  ...
  const submitArrivage = async (e) => {
    ...
    try {
      if (editLotId) {
        ...
        await axios.put(`http://192.168.100.34:3000/api/lots/update/${editLotId}`, payload, getAxiosConfig());
  ...
  const handleDeleteLot = async (id_lot) => {
    ...
    try {
      await axios.delete(`http://192.168.100.34:3000/api/lots/delete/${id_lot}`, getAxiosConfig());
