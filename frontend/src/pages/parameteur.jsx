...
function Parameteur() {
  ...
  const [taille, setTaille] = useState(""); // <-- Ajout de la taille
  ...
  const filteredAndSortedData = useMemo(() => {
    let result = data.filter((p) => {
      // FILTRE DU STATUT : Exclure "en attente" et "validé"
      const statut = (p.statut || "").toLowerCase().trim();
      if (statut === "en attente" || statut === "validé" || statut === "valide") {
        return false; 
      }
  ...
  const submit = async (e) => {
    e.preventDefault();
    // Payload mis à jour avec la taille
    const payload = { age, poids, taille, tension, temperature }; 
    try {
      if (editId) {
  ...
  const resetForm = () => {
    setNom(""); setAge(""); setPoids(""); setTaille("");
  ...
  const edit = (p) => {
    ...
    setTaille(p.taille || ""); // Récupération de la taille pour l'édition
      ...
      <form onSubmit={submit} className="card border-0 shadow-sm p-4 mb-4 no-print bg-light">
        ...
        <div className="row g-3">
          <div className="col-md-2">
            ...
            <input className="form-control bg-white" value={nom} placeholder="Sélectionnez..." disabled />
          </div>
          <div className="col-md-2">
          ...
          {/* CHAMP TAILLE RAJOUTÉ ICI */}
          <div className="col-md-2">
            <label className="small fw-bold">Taille (cm)</label>
            <input className="form-control" type="number" placeholder="ex: 175" value={taille} onChange={(e) => setTaille(e.target.value)} />
          </div>
          <div className="col-md-2">
          ...
          <table className="table table-hover align-middle mb-0">
            <thead className="table-primary text-nowrap">
              <tr>
                ...
                <th className="text-center">Taille</th> {/* COLONNE TAILLE */}
              ...
              {filteredAndSortedData.length > 0 ? (
                filteredAndSortedData.map((p) => (
                  <tr key={p.id_patient}>
                    ...
                    <td className="text-center">{p.taille ? `${p.taille} cm` : "-"}</td> {/* CELLULE TAILLE */}
                ...
                <tr><td colSpan="8" className="text-center py-4 text-muted">Aucune donnée disponible pour cette sélection.</td></tr>
              )}
            </tbody>
          </table>
        </div>
